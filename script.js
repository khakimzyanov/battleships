...
class Gamer {
	...
	reset() {
		...
		for (let key of keys) this[key].length = 0;
		for (let key in this.damage) delete this.damage[key];
	}
};
...
function dialog(c) {
	...
	if (c.btn1) {
		...
		if (fun) {
			...
			btn1.onclick = () => new Function('', 'return ' + fun)().call(this, ...pars ?? '');
		}
		if (c.btn2) {
			...
			if (fun) {
				...
				btn2.onclick = () => new Function('', 'return ' + fun2)().call(this, ...pars ?? '');
			}
		}
	}
}
...
function progress(func, ...args) {
	...
	let cnt = prsbar.children.length - 1;
	let iid = setInterval(() => {
		prsbar.children[cnt].style.opacity = 1;
		if (cnt == 0) {
			clearInterval(iid);
			setTimeout(() => {
				Array.from(prsbar.children).forEach(i => i.style.opacity = "");
...
function battle(gamer, data) {
	...
	if (target.name == 'user') {
		...
		setTimeout(progress, 1000, shot, USER);
	}
}
 ...
 * 
 * Возвращает объект масиивов ячеек.
 */
